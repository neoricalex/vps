		...
		if ( true === $strict ) :
			...
			if ( isset( $_SERVER['HTTP_COOKIE'] ) ) {
				...
				foreach ( $cookies as $cookie ) {					
					...
					$name  = trim( $parts[0] );
					if ( $name !== 'moove_gdpr_popup' && strpos( $name, 'woocommerce' ) === false ) :
	...
	public static function moove_gdpr_remove_php_cookies() {
		...
		if ( isset( $_COOKIE ) && is_array( $_COOKIE ) && $domain ) :
			foreach ( $_COOKIE as $key => $value ) {
				if ( $key !== 'moove_gdpr_popup' && strpos( $key, 'woocommerce' ) === false ) : 
					if ( 'language' === $key || 'currency' === $key ) {
						setcookie( $key, null, -1, '/', 'www.' . $domain );
					} elseif ( '_ga' === $key || '_gid' === $key || '_gat' === $key || strpos( $name, '_ga' ) !== false ) {
		...
		if ( is_array( $cookies ) ) :
			foreach ( $cookies as $cookie ) {
				$parts = explode( '=', $cookie );
				$name  = trim( $parts[0] );
				if ( $name && $name !== 'moove_gdpr_popup' && strpos( $name, 'woocommerce' ) === false ) :
					setcookie( $name, '', time() - 1000 );
					setcookie( $name, '', time() - 1000, '/' );
				endif;
			}
